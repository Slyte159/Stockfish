...
// friendly pieces.
const Score MobilityBonus[][32] = { { }, { }, { S(-65, -50), S(-42, -30), S(-9,
		-10), S(3, 0), S(15, 10), S(27, 20), // Knights
S(37, 28), S(42, 31), S(44, 33) }, { S(-52, -47), S(-28, -23), S(6, 1), S(20,
		15), S(34, 29), S(48, 43), // Bishops
S(60, 55), S(68, 63), S(74, 68), S(77, 72), S(80, 75), S(82, 77), S(84, 79), S(
		86, 81) }, { S(-47, -53), S(-31, -26), S(-5, 0), S(1, 16), S(7, 32), S(
		13, 48), // Rooks
S(18, 64), S(22, 80), S(26, 96), S(29, 109), S(31, 115), S(33, 119), S(35, 122),
		S(36, 123), S(37, 124) }, { S(-42, -40), S(-28, -23), S(-5, -7), S(0,
		0), S(6, 10), S(11, 19), // Queens
S(13, 29), S(18, 38), S(20, 40), S(21, 41), S(22, 41), S(22, 41), S(22, 41), S(
		23, 41), S(24, 41), S(25, 41), S(25, 41), S(25, 41), S(25, 41), S(25,
		41), S(25, 41), S(25, 41), S(25, 41), S(25, 41), S(25, 41), S(25, 41),
		S(25, 41), S(25, 41) } };

// Outpost[PieceType][Square] contains bonuses for knights and bishops outposts,
// indexed by piece type and square (from white's point of view).
const Value Outpost[][SQUARE_NB] = { { // A     B     C     D     E     F     G     H
		V(0), V(0), V(0), V(0), V(0), V(0), V(0), V(0), // Knights
		V(0), V(0), V(0), V(0), V(0), V(0), V(0), V(0), V(0), V(0), V(4), V(8),
				V(8), V(4), V(0), V(0), V(0), V(4), V(17), V(26), V(26), V(17),
				V(4), V(0), V(0), V(8), V(26), V(35), V(35), V(26), V(8), V(0),
				V(0), V(4), V(17), V(17), V(17), V(17), V(4), V(0) }, { V(0), V(
				0), V(0), V(0), V(0), V(0), V(0), V(0), // Bishops
		V(0), V(0), V(0), V(0), V(0), V(0), V(0), V(0), V(0), V(0), V(5), V(5),
				V(5), V(5), V(0), V(0), V(0), V(5), V(10), V(10), V(10), V(10),
				V(5), V(0), V(0), V(10), V(21), V(21), V(21), V(21), V(10), V(
						0), V(0), V(5), V(8), V(8), V(8), V(8), V(5), V(0) } };

// Threat[attacking][attacked] contains bonuses according to which piece
// type attacks which one.
const Score Threat[][PIECE_TYPE_NB] = { { S(0, 0), S(7, 39), S(24, 49), S(24,
		49), S(41, 100), S(41, 100) }, // Minor
		{ S(0, 0), S(15, 39), S(15, 45), S(15, 45), S(15, 45), S(24, 49) } // Major
};

// ThreatenedByPawn[PieceType] contains a penalty according to which piece
// type is attacked by an enemy pawn.
const Score ThreatenedByPawn[] = { S(0, 0), S(0, 0), S(80, 119), S(80, 119), S(
		117, 199), S(127, 218) };
...
const Bitboard SpaceMask[] = { (FileCBB | FileDBB | FileEBB | FileFBB)
		& (Rank2BB | Rank3BB | Rank4BB), (FileCBB | FileDBB | FileEBB | FileFBB)
...
void init_eval_info(const Position& pos, EvalInfo& ei) {
	...
	Bitboard b = ei.attackedBy[Them][KING] = pos.attacks_from<KING>(
			pos.king_square(Them));
	ei.attackedBy[Us][ALL_PIECES] = ei.attackedBy[Us][PAWN] =
			ei.pi->pawn_attacks(Us);
...
Score evaluate_pieces(const Position& pos, EvalInfo& ei, Score* mobility,
	...
	while ((s = *pl++) != SQ_NONE) {
		...
		b = Pt == BISHOP ?
			...
			Pt == ROOK ?
					attacks_bb<ROOK>(s,
							pos.pieces() ^ pos.pieces(Us, ROOK, QUEEN)) :
		...
		int mob =
				Pt != QUEEN ?
						popcount<Max15>(b & mobilityArea[Us]) :
		...
		if (Pt == ROOK) {
			...
			if (((file_of(ksq) < FILE_E) == (file_of(s) < file_of(ksq)))
					...
					&& !ei.pi->semiopen_side(Us, file_of(ksq),
		...
		if (Pt == BISHOP && pos.is_chess960()
			...
			Square d = pawn_push(Us)
					+ (file_of(s) == FILE_A ? DELTA_E : DELTA_W);
			if (pos.piece_on(s + d) == make_piece(Us, PAWN))
				score -=
						!pos.empty(s + d + pawn_push(Us)) ?
								TrappedBishopA1H1 * 4 :
	...
	return score
			- evaluate_pieces<NextPt, Them, Trace>(pos, ei, mobility,
					mobilityArea);
}

template<>
Score evaluate_pieces<KING, WHITE, false>(const Position&, EvalInfo&, Score*,
		Bitboard*) {
	return SCORE_ZERO;
}
template<>
Score evaluate_pieces<KING, WHITE, true>(const Position&, EvalInfo&, Score*,
		Bitboard*) {
	return SCORE_ZERO;
}
...
Score evaluate_king(const Position& pos, const EvalInfo& ei) {
	...
	if (ei.kingAttackersCount[Them]) {
		...
		attackUnits = std::min(20,
				(ei.kingAttackersCount[Them] * ei.kingAttackersWeight[Them])
						/ 2)
				+ 3
						* (ei.kingAdjacentZoneAttacksCount[Them]
								+ popcount<Max15>(undefended))
				+ 2 * (ei.pinnedPieces[Us] != 0) - mg_value(score) / 32;
...
Score evaluate_threats(const Position& pos, const EvalInfo& ei) {
	...
	weakEnemies = pos.pieces(Them) & ~ei.attackedBy[Them][PAWN]
...
Score evaluate_passed_pawns(const Position& pos, const EvalInfo& ei) {
	...
	while (b) {
		...
		if (rr) {
			...
			if (relative_rank(Us, blockSq) != RANK_8)
				ebonus -= square_distance(pos.king_square(Us),
						blockSq + pawn_push(Us)) * rr;
			...
			if (pos.empty(blockSq)) {
				...
				defendedSquares = unsafeSquares = squaresToQueen = forward_bb(
						Us, s);

				Bitboard bb = forward_bb(Them, s) & pos.pieces(ROOK, QUEEN)
	...
	if (Trace)
		Tracing::terms[Us][Tracing::PASSED] = apply_weight(score,
				Weights[PassedPawns]);
...
int evaluate_space(const Position& pos, const EvalInfo& ei) {
	...
	Bitboard safe =
			SpaceMask[Us] & ~pos.pieces(Us, PAWN) & ~ei.attackedBy[Them][PAWN]
					& (ei.attackedBy[Us][ALL_PIECES]
	...
	return popcount<Full>(
			(Us == WHITE ? safe << 32 : safe >> 32) | (behind & safe));
}
...
Value do_evaluate(const Position& pos) {
	...
	Bitboard mobilityArea[] = { ~(ei.attackedBy[BLACK][PAWN]
			| pos.pieces(WHITE, PAWN, KING)), ~(ei.attackedBy[WHITE][PAWN]
			| pos.pieces(BLACK, PAWN, KING)) };

	// Evaluate pieces and mobility
	score += evaluate_pieces<KNIGHT, WHITE, Trace>(pos, ei, mobility,
			mobilityArea);
	...
	if (ei.mi->game_phase() < PHASE_MIDGAME
		...
		if (pos.opposite_bishops()) {
			...
			if (pos.non_pawn_material(WHITE) == BishopValueMg
				...
				bool one_pawn = (pos.count<PAWN>(WHITE) + pos.count<PAWN>(BLACK)
						== 1);
	...
	Value v = mg_value(score) * int(ei.mi->game_phase())
			+ eg_value(score) * int(PHASE_MIDGAME - ei.mi->game_phase()) * sf
					/ SCALE_FACTOR_NORMAL;
	...
	if (Trace) {
		...
		Tracing::add_term(Tracing::SPACE, apply_weight(w, Weights[Space]),
				apply_weight(b, Weights[Space]));
...
void Tracing::format_row(std::stringstream& ss, const char* name, int idx) {
	...
	case TOTAL:
		...
		break;
	default:
		ss << std::setw(15) << name << " | " << std::noshowpos << std::setw(5)
				<< to_cp(mg_value(wScore)) << " " << std::setw(5)
				<< to_cp(eg_value(wScore)) << " | " << std::setw(5)
				<< to_cp(mg_value(bScore)) << " " << std::setw(5)
				<< to_cp(eg_value(bScore)) << " | " << std::setw(5)
				<< to_cp(mg_value(wScore - bScore)) << " " << std::setw(5)
				<< to_cp(eg_value(wScore - bScore)) << " \n";
	}
}

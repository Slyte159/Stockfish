...
Value PieceValue[PHASE_NB][PIECE_NB] = { { VALUE_ZERO, PawnValueMg,
...
PieceType min_attacker<KING>(const Bitboard*, const Square&, const Bitboard&,
		Bitboard&, Bitboard&) {
...
bool Position::pseudo_legal(const Move m) const {
	...
	if (type_of(pc) == PAWN) {
		...
		if (!(attacks_from<PAWN>(from, us) & pieces(~us) & to) // Not a capture
				...
				&& (rank_of(from) == relative_rank(us, RANK_2)) && empty(to)
	...
	if (checkers()) {
		if (type_of(pc) != KING) {
			...
			if (!((between_bb(lsb(checkers()), king_square(us)) | checkers())
					& to))
				return false;
		}
...
bool Position::gives_check(Move m, const CheckInfo& ci) const {
	...
	case PROMOTION:
		return attacks_bb(Piece(promotion_type(m)), to, pieces() ^ from)
				& ci.ksq;
	...
	case CASTLING: {
		...
		return (PseudoAttacks[ROOK][rto] & ci.ksq)
				&& (attacks_bb<ROOK>(rto,
						(pieces() ^ kfrom ^ rfrom) | rto | kto) & ci.ksq);
	}
	default:
		assert(false);
		return false;
...
void Position::do_move(Move m, StateInfo& newSt, const CheckInfo& ci,
		bool moveIsCheck) {
	...
	assert(
			piece_on(to) == NO_PIECE || color_of(piece_on(to)) == them
					|| type_of(m) == CASTLING);
	...
	if (captured) {
		...
		st->materialKey ^=
				Zobrist::psq[them][captured][pieceCount[them][captured]];
	...
	if (pt == PAWN) {
		...
		if ((int(to) ^ int(from)) == 16
			...
			st->materialKey ^=
					Zobrist::psq[us][promotion][pieceCount[us][promotion] - 1]
	...
	if (moveIsCheck) {
		if (type_of(m) != NORMAL)
			...
			if (unlikely(ci.dcCandidates) && (ci.dcCandidates & from)) {
				if (pt != ROOK)
					st->checkersBB |= attacks_from<ROOK>(king_square(them))
							& pieces(us, QUEEN, ROOK);

				if (pt != BISHOP)
					st->checkersBB |= attacks_from<BISHOP>(king_square(them))
...
template<bool Do>
void Position::do_castling(Square from, Square& to, Square& rfrom,
		Square& rto) {
...
Value Position::see(Move m) const {
	...
	do {
		...
		captured = min_attacker<PAWN>(byTypeBB, to, stmAttackers, occupied,
				attackers);
...
bool Position::is_draw() const {
	...
	StateInfo* stp = st;
	for (int i = 2, e = std::min(st->rule50, st->pliesFromNull); i <= e; i +=
			2) {
...
void Position::flip() {
	...
	f += (token == "-" ?
			token : token.replace(1, 1, token[1] == '3' ? "6" : "3"));
...
bool Position::pos_is_ok(int* step) const {
	...
	if (step && ++*step, testCastlingSquares)
		for (Color c = WHITE; c <= BLACK; ++c)
			for (CastlingSide s = KING_SIDE; s <= QUEEN_SIDE;
					s = CastlingSide(s + 1)) {
				if (!can_castle(c | s))
					continue;
				...
				if ((castlingRightsMask[king_square(c)] & (c | s)) != (c | s)
						|| piece_on(castlingRookSquare[c | s])
								!= make_piece(c, ROOK)
						|| castlingRightsMask[castlingRookSquare[c | s]]
								!= (c | s))
					return false;
			}

...
const int PushToEdges[SQUARE_NB] =
		{ 100, 90, 80, 70, 70, 80, 90, 100, 90, 70, 60, 50, 50, 60, 70, 90, 80,
				60, 40, 30, 30, 40, 60, 80, 70, 50, 30, 20, 20, 30, 50, 70, 70,
				50, 30, 20, 20, 30, 50, 70, 80, 60, 40, 30, 30, 40, 60, 80, 90,
				70, 60, 50, 50, 60, 70, 90, 100, 90, 80, 70, 70, 80, 90, 100, };

// Table used to drive the king towards a corner square of the
// right color in KBN vs K endgames.
const int PushToCorners[SQUARE_NB] = { 200, 190, 180, 170, 160, 150, 140, 130,
		190, 180, 170, 160, 150, 140, 130, 140, 180, 170, 155, 140, 140, 125,
		140, 150, 170, 160, 140, 120, 110, 140, 150, 160, 160, 150, 140, 110,
		120, 140, 160, 170, 150, 140, 125, 140, 140, 155, 170, 180, 140, 130,
		140, 150, 160, 170, 180, 190, 130, 140, 150, 160, 170, 180, 190, 200 };
...
template<typename M>
void delete_endgame(const typename M::value_type& p) {
	delete p.second;
}
...
Value Endgame<KXK>::operator()(const Position& pos) const {
	...
	Value result = pos.non_pawn_material(strongSide)
			+ pos.count<PAWN>(strongSide) * PawnValueEg + PushToEdges[loserKSq]
...
Value Endgame<KRKP>::operator()(const Position& pos) const {
	...
	// the position is drawish
	else if (rank_of(bksq) <= RANK_3 && square_distance(bksq, psq) == 1
			&& rank_of(wksq) >= RANK_4
			&& square_distance(wksq, psq)
		...
		result = Value(200)
				- 8
						* (square_distance(wksq, psq + DELTA_S)
...
Value Endgame<KRKN>::operator()(const Position& pos) const {
	...
	Value result = Value(
			PushToEdges[bksq] + PushAway[square_distance(bksq, bnsq)]);
	return strongSide == pos.side_to_move() ? result : -result;
}
...
Value Endgame<KQKR>::operator()(const Position& pos) const {
	...
	Value result = QueenValueEg - RookValueEg + PushToEdges[loserKSq]
			+ PushClose[square_distance(winnerKSq, loserKSq)];
...
ScaleFactor Endgame<KBPsK>::operator()(const Position& pos) const {
	...
	if ((pawnFile == FILE_A || pawnFile == FILE_H)
		...
		Square bishopSq = pos.list<BISHOP>(strongSide)[0];
		Square queeningSq = relative_square(strongSide,
				make_square(pawnFile, RANK_8));
	...
	if ((pawnFile == FILE_B || pawnFile == FILE_G)
		...
		if (relative_rank(strongSide, weakPawnSq) == RANK_7
				&& (pos.pieces(strongSide, PAWN)
						& (weakPawnSq + pawn_push(weakSide)))
				&& (opposite_colors(bishopSq, weakPawnSq)
...
ScaleFactor Endgame<KQKRPs>::operator()(const Position& pos) const {
	...
	if (relative_rank(weakSide, kingSq) <= RANK_2
			...
			&& (pos.pieces(weakSide, PAWN) & pos.attacks_from<KING>(kingSq)
...
ScaleFactor Endgame<KRPKR>::operator()(const Position& pos) const {
	...
	if (wpsq == SQ_A7 && wrsq == SQ_A8 && (bksq == SQ_H7 || bksq == SQ_G7)
			&& file_of(brsq) == FILE_A
			&& (rank_of(brsq) <= RANK_3 || file_of(wksq) >= FILE_D
					|| rank_of(wksq) <= RANK_5))
		return SCALE_FACTOR_DRAW;
	...
	if (r == RANK_7 && f != FILE_A && file_of(wrsq) == f && wrsq != queeningSq
		...
		return ScaleFactor(
				SCALE_FACTOR_MAX - 2 * square_distance(wksq, queeningSq));
			...
			&& (square_distance(bksq, wrsq) + tempo >= 3
					|| (square_distance(wksq, queeningSq)
							...
							&& (square_distance(wksq, wpsq + DELTA_N)
									< square_distance(bksq, wrsq) + tempo))))
		return ScaleFactor(
				SCALE_FACTOR_MAX - 8 * square_distance(wpsq, queeningSq)
...
ScaleFactor Endgame<KRPKB>::operator()(const Position& pos) const {
	...
	if (pos.pieces(PAWN) & (FileABB | FileHBB)) {
		...
		if (rk == RANK_5 && !opposite_colors(bsq, psq)) {
			...
			if (d <= 2
					&& !(d == 0 && ksq == pos.king_square(strongSide) + 2 * push))
		...
		// the corner
		if (rk == RANK_6 && square_distance(psq + 2 * push, ksq) <= 1
...
ScaleFactor Endgame<KRPPKRP>::operator()(const Position& pos) const {
	...
	if (pos.pawn_passed(strongSide, wpsq1)
			|| pos.pawn_passed(strongSide, wpsq2))
		return SCALE_FACTOR_NONE;

	Rank r = std::max(relative_rank(strongSide, wpsq1),
...
ScaleFactor Endgame<KBPKB>::operator()(const Position& pos) const {
	...
	if (file_of(weakKingSq) == file_of(pawnSq)
			&& relative_rank(strongSide, pawnSq)
					< relative_rank(strongSide, weakKingSq)
...
ScaleFactor Endgame<KBPPKB>::operator()(const Position& pos) const {
	...
	case 1:
		...
		if (ksq == blockSq1 && opposite_colors(ksq, wbsq)
				&& (bbsq == blockSq2
						|| (pos.attacks_from<BISHOP>(blockSq2)
			...
			return SCALE_FACTOR_DRAW;
				...
				&& (bbsq == blockSq1
						|| (pos.attacks_from<BISHOP>(blockSq1)
								& pos.pieces(weakSide, BISHOP))))
			return SCALE_FACTOR_DRAW;
		else
			return SCALE_FACTOR_NONE;
...
ScaleFactor Endgame<KPKP>::operator()(const Position& pos) const {
	...
	return Bitbases::probe_kpk(wksq, psq, bksq, us) ?

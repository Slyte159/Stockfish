...
ExtMove* generate_pawn_moves(const Position& pos, ExtMove* mlist,
	...
	if (Type != CAPTURES) {
		...
		if (Type == QUIET_CHECKS) {
			...
			if (pawnsNotOn7 & ci->dcCandidates) {
				dc1 = shift_bb<Up>(pawnsNotOn7 & ci->dcCandidates)
						& emptySquares & ~file_bb(ci->ksq);
	...
	if (pawnsOn7 && (Type != EVASIONS || (target & TRank8BB))) {
		...
		mlist = generate_promotions<Type, Up>(mlist, pawnsOn7, emptySquares,
				ci);
	}
	...
	if (Type == CAPTURES || Type == EVASIONS || Type == NON_EVASIONS) {
		...
		if (pos.ep_square() != SQ_NONE) {
			...
			while (b1)
				(mlist++)->move = make<ENPASSANT>(pop_lsb(&b1),
						pos.ep_square());
		}
	}
...
ExtMove* generate_all(const Position& pos, ExtMove* mlist, Bitboard target,
	...
	if (Type != CAPTURES && Type != EVASIONS && pos.can_castle(Us)) {
		if (pos.is_chess960()) {
			mlist = generate_castling<MakeCastling<Us, KING_SIDE>::right,
					Checks, true>(pos, mlist, Us, ci);
			mlist = generate_castling<MakeCastling<Us, QUEEN_SIDE>::right,
					Checks, true>(pos, mlist, Us, ci);
		} else {
			mlist = generate_castling<MakeCastling<Us, KING_SIDE>::right,
					Checks, false>(pos, mlist, Us, ci);
			mlist = generate_castling<MakeCastling<Us, QUEEN_SIDE>::right,
					Checks, false>(pos, mlist, Us, ci);
		}
	}
...
ExtMove* generate(const Position& pos, ExtMove* mlist) {
	...
	return us == WHITE ?
			generate_all<WHITE, Type>(pos, mlist, target) :
...
ExtMove* generate<QUIET_CHECKS>(const Position& pos, ExtMove* mlist) {
	...
	return us == WHITE ?
			generate_all<WHITE, QUIET_CHECKS>(pos, mlist, ~pos.pieces(), &ci) :
...
ExtMove* generate<LEGAL>(const Position& pos, ExtMove* mlist) {
	...
	Square ksq = pos.king_square(pos.side_to_move());
			...
			generate<EVASIONS>(pos, mlist) : generate<NON_EVASIONS>(pos, mlist);
	while (cur != end)
		if ((pinned || from_sq(cur->move) == ksq
				|| type_of(cur->move) == ENPASSANT)

...
unsigned index(Color us, Square bksq, Square wksq, Square psq) {
	return wksq + (bksq << 6) + (us << 12) + (file_of(psq) << 13)
...
inline Result& operator|=(Result& r, Result v) {
	return r = Result(r | v);
}
...
KPKPosition::KPKPosition(unsigned idx) {
	...
	if (square_distance(wksq, bksq) <= 1 || wksq == psq || bksq == psq
		...
		if (rank_of(psq) == RANK_7 && wksq != psq + DELTA_N
				&& (square_distance(bksq, psq + DELTA_N) > 1
						|| (StepAttacksBB[KING][wksq] & (psq + DELTA_N))))
			result = WIN;
	}
	// Immediate draw if it is a stalemate or a king captures undefended pawn
	else if (!(StepAttacksBB[KING][bksq]
			& ~(StepAttacksBB[KING][wksq] | StepAttacksBB[PAWN][psq]))
...
Result KPKPosition::classify(const std::vector<KPKPosition>& db) {
	...
	while (b)
		r |= Us == WHITE ?
				db[index(Them, bksq, pop_lsb(&b), psq)] :

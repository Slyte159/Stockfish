...
extern "C" {
long start_routine(ThreadBase* th) {
	th->idle_loop();
	return 0;
}
}
...
void Thread::split(Position& pos, const Stack* ss, Value alpha, Value beta,
		Value* bestValue, Move* bestMove, Depth depth, int moveCount,
		MovePicker* movePicker, int nodeType, bool cutNode) {

	assert(pos.pos_is_ok());
	assert(
			-VALUE_INFINITE < *bestValue && *bestValue <= alpha && alpha < beta
					&& beta <= VALUE_INFINITE);
...
void ThreadPool::start_thinking(const Position& pos, const LimitsType& limits,
	...
	for (MoveList<LEGAL> it(pos); *it; ++it)
		if (limits.searchmoves.empty()
				|| std::count(limits.searchmoves.begin(),
						limits.searchmoves.end(), *it))

...
inline Bitboard Position::attacks_from(Square s) const {
	...
	return Pt == BISHOP || Pt == ROOK ?
			...
			Pt == QUEEN ?
					attacks_from<ROOK>(s) | attacks_from<BISHOP>(s) :
					StepAttacksBB[Pt][s];
}
...
inline bool Position::opposite_bishops() const {
	...
	return pieceCount[WHITE][BISHOP] == 1 && pieceCount[BLACK][BISHOP] == 1
			&& opposite_colors(pieceList[WHITE][BISHOP][0],
					pieceList[BLACK][BISHOP][0]);
}
...
inline void Position::move_piece(Square from, Square to, Color c,
		PieceType pt) {

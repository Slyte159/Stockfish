...
inline bool has_positive_value(const ExtMove& ms) {
	return ms.value > 0;
}
...
MovePicker::MovePicker(const Position& p, Move ttm, Depth d,
		const HistoryStats& h, Move* cm, Move* fm, Search::Stack* s) :
...
void MovePicker::score<CAPTURES>() {
	...
	for (ExtMove* it = moves; it != end; ++it) {
		...
		if (type_of(m) == ENPASSANT)
			...
			it->value += PieceValue[MG][promotion_type(m)]
					- PieceValue[MG][PAWN];
	}
}
...
Move MovePicker::next_move<false>() {
	...
	while (true) {
		...
		case KILLERS_S1:
			move = (cur++)->move;
			if (move != MOVE_NONE && move != ttMove && pos.pseudo_legal(move)
					&& !pos.capture(move))
				return move;
			break;
		...
		case QUIETS_2_S1:
			move = (cur++)->move;
			if (move != ttMove && move != killers[0].move
					&& move != killers[1].move && move != killers[2].move
					&& move != killers[3].move && move != killers[4].move
					&& move != killers[5].move)
				return move;
			break;
